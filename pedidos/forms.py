...
class PedidoVentaBaseForm(forms.Form):
    variante = forms.ModelChoiceField(queryset=VarianteProducto.objects.select_related('producto').order_by('sku'))
    cantidad = forms.IntegerField(min_value=1)
    direccion_linea1 = forms.CharField(max_length=160)
    ciudad = forms.CharField(max_length=80)
    departamento = forms.CharField(max_length=80, required=False)
    codigo_postal = forms.CharField(max_length=20, required=False)
    nombre_destinatario = forms.CharField(max_length=120, required=False)
    estado = forms.ChoiceField(choices=PedidoVenta.ESTADOS, initial='PENDIENTE_PAGO')
...
class PedidoVentaForm(PedidoVentaBaseForm):
    cliente = forms.ModelChoiceField(queryset=Cliente.objects.select_related('usuario').order_by('usuario__correo'))
